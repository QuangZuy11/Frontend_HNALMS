...
import { ROLE_LABELS, STATUS_LABELS, formatAccountDate, type AccountDetail } from '../constants';
import '../account-management.css';

export default function ManagerAccountDetail() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setLoading(true);
        setError(null);
        const response = await accountService.detail('managers', id);
  ...
  const handleDisable = async () => {
    if (!id || !window.confirm('Bạn có chắc muốn đóng tài khoản này?')) return;
    try {
      setDisablingId(true);
      const response = await accountService.disable('managers', id);
  ...
  const handleEnable = async () => {
    ...
    try {
      setDisablingId(true);
      const response = await accountService.enable('managers', id);
      if (response.data) setAccount((prev) => (prev ? { ...prev, status: response.data.status } : prev));
    } catch (err) {
      const errObj = err as { response?: { data?: { message?: string } } };
      ...
      <div className="created-accounts-card">
        <div className="created-accounts-header">
          <div>
            ...
            <p className="created-accounts-subtitle">{account.username} · {ROLE_LABELS[account.role] || account.role}</p>
          </div>
          <Link to="/owner/accounts" className="btn-secondary">← Danh sách</Link>
        </div>
        <div className="detail-content">
          <div className="detail-section">
            ...
            <div className="detail-row"><span className="detail-label">Vai trò:</span><span className="role-badge">{ROLE_LABELS[account.role] || account.role}</span></div>
